...
# APIs
使用API：
`local qk = require("quicklib")`

创建控件：  
`qk.new(包含初始化数据的表:table,控件元表:table)`  
...
dx=5|控件的x方向长度  
dy=1|控件的y方向长度  
rf=1（0.1的倍数）|刷新间隔时间。当rf=0时不会执行绘制函数，是`qk.hide()`的原理   
...
### 对控件属性进行直接编辑：
表`qk.data`记录着所有创建过的控件，控件的id值即为控件在此表的索引。  
通过此表可以直接访问或修改控件的属性，例：`qk.data[控件的id][属性名称]`  
在使用`qk.del`删除控件时只会把控件的type类型改为"n"，不可直接赋值nil。  
### 关于pic表：
